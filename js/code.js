const urlBase =
  ...
  (window.location.hostname === "localhost" ||
    ...
    window.location.origin.includes("finnick"))
    ? "/api/index.php"
    : "http://lamp.finnick.party/api/index.php";
...
const loginUrlBase =
  ...
  (window.location.hostname === "localhost" ||
    ...
    window.location.origin.includes("finnick"))
    ? "/api/login.php"
    : "http://lamp.finnick.party/api/login.php";

const registerUrlBase =
  typeof window !== "undefined" &&
  window.location &&
  (window.location.hostname === "localhost" ||
    window.location.hostname === "127.0.0.1" ||
    window.location.origin.includes("finnick"))
    ? "/api/register.php"
    : "http://lamp.finnick.party/api/register.php";
...
function doLogin() {
  ...
  console.log("This is the current package: ", jsonPayload);
...
function doRegister() {
  ...
  let fullName = loginInput ? fullNameInput.value.trim() : "";
  let emailInput = document.getElementById("loginEmail");
  let email = emailInput ? emailInput.value.trim() : "";

  document.getElementById("loginResult").innerHTML = "";

  let jsonPayload = JSON.stringify({ username: login, email: email, password: password, fullName: fullName });
...
function saveCookie() {
  ...
  // Build the suffix once
  let cookieOptions = ";expires=" + date.toGMTString() + ";path=/;SameSite=Lax";

  // Append it to each cookie assignment
...
function addColor() {
  ...
  if (!newColor) {
    ...
    resultEl.innerHTML =
      "<i class='bi bi-exclamation-triangle-fill me-1'></i> Please enter a color name";
    return;
  }
    ...
    xhr.onreadystatechange = function () {
      if (this.readyState === 4) {
        if (this.status === 201 || this.status === 200) {
          ...
          resultEl.innerHTML =
            "<i class='bi bi-check-circle-fill me-1'></i> Color successfully added!";
          newColorInput.value = "";
          searchColor();
        } else {
          try {
  ...
  try {
    xhr.onreadystatechange = function () {
      if (this.readyState === 4 && this.status === 200) {
        resultSpan.innerHTML =
          "<i class='bi bi-check-circle me-1'></i> Results updated";
        let jsonObject = JSON.parse(xhr.responseText);
        let targetP =
        ...
        if (colors.length === 0 || jsonObject.error === "No Records Found") {
          if (targetP)
            targetP.innerHTML = `<div class="text-secondary-contrast small italic py-2"><i class="bi bi-info-circle me-1"></i> No matching colors found.</div>`;
          return;
        }
...
function addContact() {
  let newContactInput = document.getElementById("contactLookup");
  ...
  if (!newContact) {
    resultEl.className = "text-warning small fw-semibold";
    resultEl.innerHTML =
      "<i class='bi bi-exclamation-triangle-fill me-1'></i> Please enter a contact's email";
    return;
  }
  ...
  try {
    xhr.onreadystatechange = function () {
      if (this.readyState === 4 && this.status === 200) {
        ...
        if (colors.length === 0 || jsonObject.error === "No Records Found") {
          if (targetP)
            targetP.innerHTML = `<div class="text-secondary-contrast small italic py-2"><i class="bi bi-info-circle me-1"></i> No matching contacts found.</div>`;
          return;
        }
